#!/usr/bin/env python3
"""
Script completo RAG con Code2Vec su TUTTI i file PrimeVul (con PAGINAZIONE)
...
def retry_with_exponential_backoff(func, max_retries=3, initial_delay=5, max_delay=120):
    """
...
def analyze_with_gemini(test_example, relevant_documents):
    ...
    return retry_with_exponential_backoff(make_gemini_call, max_retries=3, initial_delay=5, max_delay=120)
...
def analyze_vulnerability(supabase, test_example, all_training_docs):
    ...
    try:
        response = supabase.rpc('find_primevul_result_c2v_by_code', {
    ...
    try:
        supabase.table("primevul_results_c2v").insert(result_record).execute()
...
def main():
    ...
    try:
        ...
        if training_count.count == 0 or test_count.count == 0:
            print("Database non popolato! Esegui prima load_precomputed_c2v_primevul.py")
            return
    ...
    # Carico training set una volta sola, con paginazione per PrimeVul
    ...
    try:
        ...
        while True:
            page = supabase.table('primevul_training').select('*').range(offset, offset + page_size - 1).execute()
            if not page.data:
                break
            all_training_docs.extend(page.data)
            offset += page_size
    ...
    # Carico test set una volta sola, con paginazione per PrimeVul
    ...
    try:
        ...
        while True:
            page = supabase.table('primevul_test').select('*').range(offset, offset + page_size - 1).execute()
            if not page.data:
                break
            test_vulnerabilities.extend(page.data)
            offset += page_size
            if len(page.data) < page_size:
                break
        ...
        try:
            ...
            if idx < len(test_vulnerabilities): 
                if status == 'skipped':
                    ...
                    time.sleep(2)  # 2 secondi per chiamate API (success o error)
